...
class SessionSchemaMixin:
    ...
    def _init_schema(self):
        ...
        if fts5_available:
            ...
            # Fork gate: decide trigram participation before any FTS branch so
            # both the legacy and v23 paths agree on the expected trigger set.
            # Both helpers are individually guarded, so this stays safe even on
            # a database whose indexes are already stale/corrupt.
            ...
            if self._fts_stale:
                ...
                if self._fts_enabled and triggers_need_repair:
                    self._rebuild_legacy_fts_indexes(
                        cursor, include_trigram=bool(trigram_allowed and self._trigram_available)
                    )
            else:
                self._fts_enabled = self._ensure_fts_schema(
                ...
                if self._fts_enabled and triggers_need_repair:
                    self._rebuild_fts_indexes(
                        cursor,
                        include_trigram=bool(trigram_allowed and self._trigram_available),
                    )
