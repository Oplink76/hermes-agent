...
TERMINAL_KINDS = ("completed", "blocked", "gave_up", "crashed", "timed_out", "status", "needs_ole", "archived", "unblocked", "block_loop_detected", "review_requested", "changes_requested")
...
def _fmt_needs_ole(ev, n) -> tuple:
    payload = ev.payload or {}
    resolver = str(payload.get("resolver_profile") or "resolver")[:80]
    diagnosis = str(payload.get("diagnosis") or "Resolver repair applied")[:240]
    return (
        f"⚠️ {n.board_tag}Kanban {n.task_id} Resolver audit — {n.title}\n"
