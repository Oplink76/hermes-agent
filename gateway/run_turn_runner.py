...
class TurnRunner:
    ...
    def run_sync(self):
        ...
        try:
            ...
            profile_name = getattr(ctx.source, "profile", None) or runner._active_profile_name()
            provider_name = None
            if ctx.session_key:
                override = runner._session_model_overrides.get(ctx.session_key)
                if isinstance(override, dict):
                    provider_name = override.get("provider")
            if not provider_name:
                model_config = ctx.user_config.get("model") if isinstance(ctx.user_config, dict) else None
                provider_name = model_config.get("provider") if isinstance(model_config, dict) else None
            logger.warning("Gateway runtime resolution failed for profile=%s provider=%s: %s",
                           profile_name, provider_name or "(configured)", exc)
            return {"final_response": _gateway_auth_route_failure_reply(profile_name, provider_name),
