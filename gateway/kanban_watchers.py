...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        stored cursor with a subscribed notification kind: the terminal set
        (``completed``, ``blocked``, ``gave_up``, ``crashed``, ``timed_out``,
        ``block_loop_detected``) plus the non-terminal ``review_requested``
        lane change and the ``needs_ole`` repair audit. Sends one
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        # cycles keep notifying. ``needs_ole`` is the fork's Resolver repair
        # audit — also non-terminal, and notified for the same reason.
        TERMINAL_KINDS = (
            "completed", "blocked", "gave_up", "crashed", "timed_out",
