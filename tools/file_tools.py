...
def extract_v4a_patch_endpoints(patch: str) -> list[tuple[str, str]]:
    """Return ``(operation, endpoint)`` for every endpoint in a V4A patch.

    ``operation`` is the lowercase V4A op name (``add`` / ``update`` /
    ``delete`` / ``move``). A Move contributes BOTH of its endpoints, each
    tagged ``move``. Callers use the op to decide whether an endpoint is also
    a CONTENT write (Update/Add) as opposed to a path-only touch.
    """
    ...
    for operation in operations:
        op_name = operation.operation.value
        endpoints.append((op_name, operation.file_path))
        if operation.new_path:
            endpoints.append((op_name, operation.new_path))
    return endpoints


def extract_v4a_patch_paths(patch: str) -> list[str]:
    """Return every filesystem endpoint parsed from a V4A patch."""
    return [path for _op, path in extract_v4a_patch_endpoints(patch)]
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    if mode == "patch" and patch:
        ...
        # operations the file tool will execute. The op also decides whether an
        # endpoint is a CONTENT write: Update/Add write text, while Delete and
        # Move endpoints are path-only and skip the binary-document guard.
        for _v4a_op, v4a_path in extract_v4a_patch_endpoints(patch):
            _err = _reject_v4a_traversal(v4a_path)
            if _err:
                return _err
            _paths_to_check.append(v4a_path)
            if _v4a_op in ("update", "add"):
