...
def _human_decision(spec: _GateSpec, *, command: str, description: str,
                    ...
                    permanent_capable: bool = True, pending_body=None, one_shot: bool = False) -> dict:
    ...
    def grant(choice: str) -> dict:
        if one_shot:
            if choice != "once":
                return _blocked(
                    "BLOCKED: This governed operation requires an exact one-shot approval; "
                    "session and permanent approvals are invalid.",
                    pattern_key=pattern_key, description=description,
                )
            return {"approved": True, "message": None,
                    "actor": "human:gateway" if is_gateway or is_ask else "human:cli"}
    ...
    if spec.transport:
        attempt = _present_with_selected_transport(
            ...
            allow_session=not smart_denied and not one_shot, allow_permanent=allow_permanent,
        )
...
def _run_approval_gate(
    *, pattern_key: str, description: str, display_target: str, approval_callback=None,
    ...
    if not one_shot and _yolo_active():
        return _approved()
    ...
    if one_shot and not is_cli and not is_gateway:
        return _blocked(
            "BLOCKED: This governed operation requires an exact one-shot approval from a human.",
            pattern_key=pattern_key, description=description,
        )
    ...
    return _human_decision(
        ...
        approval_callback=approval_callback, is_cli=is_cli, is_gateway=is_gateway, is_ask=is_ask, one_shot=one_shot,
    )
...
def request_tool_approval(tool_name: str, reason: str, *, rule_key: str = "", approval_callback=None,
                          one_shot_override: Optional[dict] = None) -> dict:
