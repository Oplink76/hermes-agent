...
def _run_approval_gate(
    *,
    ...
    if not is_cli and not is_gateway:
        # Fork invariant: an exact one-shot approval is fail-closed without a
        # present human. This is checked BEFORE the single-query mode below so
        # `single_query_mode: approve` can never auto-approve an operation that
        # requires explicit, human, one-shot authorization.
        if one_shot:
            logger.warning(
