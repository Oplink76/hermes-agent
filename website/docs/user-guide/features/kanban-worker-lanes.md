...
The injected `KANBAN_GUIDANCE` describes the product-board lifecycle (Development ends with `kanban_complete` — commit-first, so Hermes commits the diff and Test/Review consume that candidate) alongside both non-product graph shapes: `kanban_complete` for truly terminal tasks (typo fixes, docs changes, research writeups), the same-card review loop, and `kanban_block` for genuine blockers.
