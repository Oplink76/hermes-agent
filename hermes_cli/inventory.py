...
def build_models_payload(
    ...
    # Fork local-agent providers remain discoverable before their prerequisites are installed.
    rows = list(rows) + [row for row in _append_unconfigured_rows(rows, ctx)
...
def _append_unconfigured_rows(
    ...
    from hermes_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery, _PROVIDER_MODELS
    ...
    extras: list[dict] = []
    excluded = {str(slug).strip().lower() for slug in (ctx.excluded_providers or [])}
    for entry in CANONICAL_PROVIDERS:
        provider_cfg = ctx.user_providers.get(entry.slug) if isinstance(ctx.user_providers, dict) else None
        if entry.slug.lower() in seen or entry.slug.lower() in excluded or not is_provider_enabled(provider_cfg):
            continue
        if current_only and entry.slug.lower() != cur:
            continue
        if entry.slug in {"claude-cli", "codex-cli", "cowork"}:
            prerequisite = {"claude-cli": "`claude` executable", "codex-cli": "`codex` executable",
                            "cowork": "configured Cowork MCP `cowork_run` tool"}[entry.slug]
            models = ([cur_model] if cur_model else []) if entry.slug.lower() == cur else list(_PROVIDER_MODELS.get(entry.slug, ["default"]))
            extras.append(_canonical_row(entry, cur, models=models, total_models=len(models),
                source="configured-current" if entry.slug.lower() == cur else "canonical", authenticated=False,
                auth_type="external_process", key_env="", warning=f"Unavailable: requires {prerequisite}; no API key is used."))
            continue
