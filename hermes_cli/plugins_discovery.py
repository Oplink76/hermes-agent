...
def gate_manifest(
    ...
    gate_enabled = _manifest_config_gate_enabled(manifest.config_gate) if manifest.config_gate else True
    allowlisted = enabled is not None and bool(names & enabled)
    is_enabled = gate_enabled if manifest.source == "bundled" and manifest.config_gate else allowlisted and gate_enabled
    if not is_enabled:
        if manifest.config_gate and not gate_enabled:
            return _placeholder(
                f"not enabled in config (set {manifest.config_gate}: true to activate)",
                logging.DEBUG, "Skipping '%s' (config gate disabled)",
            )
        return _placeholder(
