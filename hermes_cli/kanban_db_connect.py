...
class _KanbanConnection(sqlite3.Connection):
    """SQLite connection with callbacks released only at durable boundaries."""
    ...
    def _run_post_commit_callbacks(self) -> None:
        ...
        for callback in callbacks:
            try:
                ...
                _kb._log.warning("Post-commit callback failed: %s", exc)
...
def _register_governance_write_authorizer(conn: sqlite3.Connection) -> None:
    conn.create_function(
        ...
        0,
        lambda: 1 if _kb._GOVERNANCE_WRITE_AUTHORIZED.get() else 0,
    )
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    path = db_path if db_path is not None else _kb.kanban_db_path(board=board)
    governance_board = None if db_path is not None else (
        (_kb._normalize_board_slug(board) if board is not None else _kb.get_current_board())
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        if schema_present:
            _kb._sync_board_governance(conn, _kb._known_board_slug_for_connection(conn) or governance_board)
            return conn
...
_LATER_TASK_COLUMNS = (
    ...
    ("work_contract_id", "work_contract_id TEXT"),
    ("work_item_kind", "work_item_kind TEXT NOT NULL DEFAULT 'card'"),
    ("source_commit_required", "source_commit_required INTEGER NOT NULL DEFAULT 0"),
    ("source_commit_forbidden", "source_commit_forbidden INTEGER NOT NULL DEFAULT 0"),
    ...
    ("rework_count", "rework_count INTEGER NOT NULL DEFAULT 0"),
    ("running", "running INTEGER NOT NULL DEFAULT 0"),
    ("blocked", "blocked INTEGER NOT NULL DEFAULT 0"),
)
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    _kb._migrate_qualification_intake_lifecycle(conn)
    _kb._ensure_qualification_boundary_objects(conn)
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    """IMMEDIATE write transaction with the fork's composable savepoints.

    The fork deliberately permits nested calls: dashboard snapshot guards and
    governance mutations hold one outer transaction across canonical helpers.
    ``allow_nested`` remains accepted to document upstream caller intent.
    Annotating those fork callers and separating their post-commit effects is
    existing convergence work, outside this upstream integration repair.
    """
