...
async def check_hermes_update(force: bool = False):
    ...
    if install_method == "apt":
        ...
        # Wrapped like every other return in this function: the fork's
        # dashboard contract is that check_hermes_update never returns a
        # payload without upstream sync status. Upstream has no such
        # decorator, so its new branch returns payload bare.
        return _with_upstream_sync_status(payload)
