...
def _cleanup_workspace(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    try:
        ...
        if kind == "worktree" and path:
            ...
            if not _kb._worktree_has_other_running_consumer(conn, task_id, worktree_path):
                _kb._cleanup_provisioned_node_dependencies(worktree_path)
            # Fork worktrees persist for later source/review handoffs.
...
def resolve_workspace(
    ...
    if kind == "worktree":
        return _kb._resolve_worktree_workspace(task, board=board, conn=conn)[0]
...
def _set_task_column(conn: sqlite3.Connection, task_id: str, column: str, value: str) -> None:
    _kb._validate_resolver_cas_fields({column: value})
    with _kb.write_txn(conn):
