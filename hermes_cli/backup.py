...
_QUICK_STATE_FILES = (
    "state.db", "config.yaml", ".env", "auth.json", SIGNING_KEY_RELATIVE_PATH, "cron/jobs.json", "cron/executions.db",
...
def _quick_shared_state_root(home: Path) -> Path:
    """Resolve the shared signing-key root for a default or named profile."""
...
def _quick_snapshot_candidates(home: Path):
    ...
    for rel in _QUICK_STATE_FILES:
        src_root = _quick_shared_state_root(home) if rel == SIGNING_KEY_RELATIVE_PATH else home
        src = src_root / rel
        if src.is_dir():
...
def restore_quick_snapshot(snapshot_id: str, hermes_home: Optional[Path] = None) -> bool:
    ...
    for rel in meta.get("files", {}):
        src = snap_dir / rel
        dst_root = _quick_shared_state_root(home) if rel == SIGNING_KEY_RELATIVE_PATH else home
        dst = dst_root / rel
        if not (_is_within(src, snap_res) and _is_within(dst, dst_root.resolve())):
