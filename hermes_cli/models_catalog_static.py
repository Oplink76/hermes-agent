...
CANONICAL_PROVIDERS: list[ProviderEntry] = [ProviderEntry(*row) for row in (
    ...
    ("claude-cli", "Claude Code CLI (local agent)", "Local Claude Code agent in the active project; native actions use Claude permissions, outside Hermes per-tool approvals"),
    ("codex-cli", "Codex CLI (local agent)", "Local Codex agent in the active project; native actions use the Codex sandbox, outside Hermes per-tool approvals"),
    ("cowork", "Cowork (local agent)", "Configured Cowork MCP agent with installed skills; native actions are outside Hermes per-tool approvals"),
)]
