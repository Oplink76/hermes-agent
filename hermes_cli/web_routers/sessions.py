...
async def search_sessions(
    ...
    return await _run_session_db_io(_search)
...
async def bulk_delete_sessions_endpoint(body: BulkDeleteSessions):
    ...
    deleted = await _run_session_db_io(
...
async def import_sessions_endpoint(request: Request):
    ...
    try:
        result = await _run_session_db_io(_with_db, body.profile, lambda db: db.import_sessions(body.sessions), read_only=False)
    except ValueError as exc:
...
async def count_empty_sessions_endpoint(profile: Optional[str] = None):
    ...
    count = await _run_session_db_io(
...
async def get_session_stats(profile: Optional[str] = None):
    ...
    return await _run_session_db_io(_with_db, profile, _stats, read_only=True)
...
async def get_session_messages(
    ...
    result = await _run_session_db_io(_with_db, profile, _read, read_only=True)
    if result is None:
        raise HTTPException(status_code=404, detail=_NOT_FOUND)
...
async def delete_session_endpoint(session_id: str, profile: Optional[str] = None):
    ...
    return await _run_session_db_io(_with_db, profile, _delete, read_only=False)
...
async def backfill_session_owner_profiles(body: SessionOwnerBackfill):
    ...
    with http_failure(
        ...
        stamped = await _run_session_db_io(
...
async def rename_session_endpoint(session_id: str, body: SessionRename):
    """``title`` renames (empty/null clears); ``archived`` soft-hides or
...
async def export_session_endpoint(session_id: str, profile: Optional[str] = None):
    ...
    prepared = await _run_session_db_io(_with_db, profile, _prepare_export, read_only=True)
