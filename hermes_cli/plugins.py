...
def _get_pre_tool_call_directive_details(
    ...
    # The fork keeps scanning after an approve (to prefer a one-shot override
    # over a plain approval), so a directive built mid-loop can predate later
    # ``modify`` hooks. Re-attach the fully accumulated args at the single
    # return point; the directive is a frozen dataclass, hence ``replace``.
    final = first_one_shot_approval or first_approval or _PreToolCallDirective()
    if modified_args is not None and final.modified_args is not modified_args:
        final = replace(final, modified_args=modified_args)
    return final
...
def _dispatch_pre_tool_call_hooks(
    ...
    block_msg = _resolve_block_from_details(
        details, tool_name, args=details.modified_args if details.modified_args is not None else args,
