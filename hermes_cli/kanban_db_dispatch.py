...
def enforce_max_runtime(conn: sqlite3.Connection, *, signal_fn=None) -> list[str]:
    ...
    for row in rows:
        ...
        with _kb.write_txn(conn):
            ...
            if cur.rowcount == 1:
                _kb._apply_v2_flags_for_status(conn, tid, retry_status)
                payload = {
                    "pid": pid,
...
def _reclaim_dead_workers(conn: sqlite3.Connection) -> _CrashSweep:
    ...
    with _kb.write_txn(conn):
        ...
        for row in rows:
            ...
            if cur.rowcount != 1:
                continue
            _kb._apply_v2_flags_for_status(conn, row["id"], retry_status)
            run_id = _kb._end_run(
                conn, row["id"],
...
def _account_crashes(conn: sqlite3.Connection, crash_details: list) -> list[str]:
    ...
    product_board = _kb.product_board_metadata(_kb._board_slug_for_connection(conn)) is not None
    for tid, pid, claimer, protocol_violation, error_text in crash_details:
        if protocol_violation and product_board:
            tripped = _record_task_failure(
                conn, tid, error=error_text, outcome="crashed",
                failure_limit=1, force_trip=True, release_claim=False,
                end_run=False, event_payload_extra={"pid": pid, "claimer": claimer},
            )
            if tripped:
    ...
    with _kb.write_txn(conn):
        ...
        if not (force_trip or failures >= effective_limit):
            if release_claim:
                ...
                if cur.rowcount == 1:
                    _kb._apply_v2_flags_for_status(conn, task_id, retry_status)
            else:
                conn.execute(
        ...
        if cur.rowcount == 1:
            _kb._apply_v2_flags_for_status(conn, task_id, "blocked")
        payload = {
            "failures": failures,
...
def _dispatch_lane_task(
    ...
    *,
    ...
    """
    task_id = row["id"]
    routing_error = _kb.resolver_routing_error(conn, task_id, assignee)
    if dry_run and routing_error is not None:
    ...
    if claimed is None:
        return False
    try:
        _kb._stamp_run_executor_identity(conn, claimed)
    ...
    try:
        ...
        if claimed.workspace_kind == "worktree":
            workspace, resolved_branch_name = _kb._resolve_worktree_workspace(
                claimed, board=board, base_branch=_kb._story_base_branch(conn, task_id, board=board), conn=conn)
        else:
            workspace = kanban_db_workspace.resolve_workspace(claimed, board=board, conn=conn)
    except Exception as exc:
        if _record_task_failure(
    ...
    if not _kb._pin_test_target_or_block(conn, claimed, workspace, board=board):
        result.auto_blocked.append(claimed.id)
        return False
    if not _kb._pin_review_target_or_block(
        conn, claimed, workspace, board=board, default_review_status=lane == "review"
    ):
        result.auto_blocked.append(claimed.id)
        return False
    try:
...
def _worker_argv(task: Task, profile_arg: str, hermes_home: Optional[str]) -> list[str]:
    ...
    worker_toolsets = (["resolver_readonly"] if profile_arg == "resolver"
