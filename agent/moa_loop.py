...
def _resolve_preset_cached(preset_name: str) -> tuple[dict[str, Any], Any]:
    ...
    _reject_raw_cowork_slots(moa_raw, preset_name)
...
def _run_reference(
    ...
    is_cancelled: Any = None, cache_disabled: bool | None = None, cache_ttl: str | None = None,
) -> tuple[str, str, Any]:
    ...
    trace_fields = {"model": slot.get("model"), "provider": slot.get("provider"), "temperature": temperature}
    # The advisory view already stripped the agent's system prompt; this is the only one.
    messages = [{"role": "system", "content": _REFERENCE_SYSTEM_PROMPT}, *ref_messages]
    try:
        runtime = _slot_runtime(slot)
        trace_fields["provider"] = runtime.get("provider") or slot.get("provider")
        ...
        response = call_llm(
            ...
            extra_headers={"x-initiator": "user"} if is_copilot else None, is_cancelled=is_cancelled, **runtime,
        )
        ...
        note = f"[failed: {exc}]"
        return label, note, _RefAccounting(CanonicalUsage(request_count=0), messages=messages, output=note, **trace_fields)
...
def _placeholder_output(slot: dict[str, Any], note: str) -> tuple[str, str, Any]:
    ...
    return _slot_label(slot), note, _RefAccounting(CanonicalUsage(request_count=0))
    ...
    try:
        for idx, slot in enumerate(reference_models):
            ...
            futures[executor.submit(
                ...
                cache_disabled=cache_disabled, cache_ttl=cache_ttl, is_cancelled=is_cancelled,
            )] = idx
    ...
    *, user_prompt: str, api_messages: list[dict[str, Any]], reference_models: list[dict[str, Any]],
    ...
    try:
        ...
        synthesis = _extract_text(call_llm(
            ...
            reasoning_config=_aggregator_reasoning_config(aggregator), is_cancelled=_agent_cancel_check(agent), **agg_runtime,
        ))
    except Exception as exc:
...
class MoAChatCompletions:
    ...
    def _fold_pending_accounting(self, usage: Any, cost: Any) -> None:
        ...
        with self._accounting_lock:
            self._pending_reference_usage = (self._pending_reference_usage or CanonicalUsage(request_count=0)) + usage
            if cost is not None:
