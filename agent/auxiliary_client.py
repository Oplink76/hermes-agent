...
def _call_llm_impl(
    ...
    if task in {"moa_reference", "moa_aggregator"} or provider in CLI_EMULATED_ROUTES or str(base_url or "").startswith("cli://"):
        resolved_provider, resolved_model, resolved_base_url, _, _ = _resolve_task_provider_model(
            task, provider, model, base_url, api_key)
