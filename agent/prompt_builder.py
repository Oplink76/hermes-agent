...
KANBAN_GUIDANCE = (
    ...
    "kind=\"needs_input\", attempted_resolutions=[\"implemented\", "
    "\"verified\", \"posted artifacts\"])` "
    "so a reviewer can approve+unblock or request changes. Reviewing-then-"
    "completing is more honest than auto-completing work that still needs "
    "eyes on it. On product boards, code handoffs are commit-first: Hermes "
    "commits source changes. Leave code uncommitted — do not `git commit` "
    "it yourself or source handoffs won't advance. Test/review evidence can "
    "complete with no diff. Never `git push`.\n"
...
RESOLVER_KANBAN_GUIDANCE = (
    ...
    "When the board requires Agent Memory, call `kanban_agent_memory_recall` "
    "before deciding and `kanban_agent_memory_write` after diagnosing; include "
    "receipts when available in `kanban_resolve` — never run `hermes "
    "agent-memory`. Missing or failed memory is advisory: still "
    "call `kanban_resolve` without replaying diagnosis. "
