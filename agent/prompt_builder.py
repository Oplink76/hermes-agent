...
KANBAN_GUIDANCE = (
    ...
    "include the structured handoff on the lifecycle transition itself. "
    ...
    "only \u2014 and never request same-card review as well; either "
    ...
    "   **Product workflow Development ends with `kanban_complete`.** Code "
    "handoffs there are commit-first: Hermes commits your source changes at "
    "completion, and that commit is the immutable candidate SHA the "
    "downstream Test and Review steps read. Leave code uncommitted \u2014 do not "
    "`git commit` it yourself or the source handoff won't advance. Never "
    "`git push`. Test/review evidence can complete with no diff. Completing "
    "is a handoff, not self-approval \u2014 Review is the approval gate. "
