...
KANBAN_GUIDANCE = (
    ...
    "`kanban_show(task_id=...)` before choosing the "
    "terminal action. When any pre-created review, QA, or release child "
    "depends on your task, call `kanban_complete` \u2014 completion is what "
    "releases those children. Never sticky-block that parent for "
    "`review-required` \u2014 that convention exists outside the product workflow "
    "only \u2014 and never request same-card review as well; either strands or "
    "duplicates the downstream lane. "
