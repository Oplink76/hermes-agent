...
def build_memory_guidance(memory_enabled: bool = True, profile_enabled: bool = True) -> str:
    ...
    return frame + (
        ...
        "Memory is the narrow exception for durable facts that apply to EVERY "
...
4. **Block only after exhausting options.** Before a human/capability block, try safe alternatives: local docs/code, workspace search, fallback tools, peer comments, and question narrowing. For product boards, call `kanban_block(reason="...", kind="needs_input", attempted_resolutions=[...])`. Hermes takes the first resolution pass; only if Hermes cannot resolve does the card enter `Blocked` and notify Slack/human. Don't guess or block just because the first path failed.
5. **Finish with the review model encoded by the task graph.** Carry the structured handoff on the lifecycle transition itself: `summary` is 1–3 sentences naming concrete artifacts, `metadata` is machine-readable facts (`{changed_files: [...], tests_run: N, decisions: [...]}`). Downstream workers read both via `kanban_show`. Never put secrets / tokens / raw PII in either — run rows are durable forever. If `kanban_show()` lists child IDs, inspect those cards with `kanban_show(task_id=...)` before choosing the terminal action. When any pre-created review, QA, or release child depends on your task, call `kanban_complete` — completion is what releases those children. Never sticky-block that parent for `review-required` — that convention exists outside the product workflow only — and never request same-card review as well; either strands or duplicates the downstream lane. Otherwise, when this same task needs review before it is final, call `kanban_request_review(summary=..., metadata=..., reviewer=<optional-profile>)`. The reviewer approves with `kanban_complete`, returns rework with `kanban_request_changes`, or uses `kanban_block` only for a genuine external escalation. Review is not a block, so repeated review cycles never trip unblock-loop detection.
   **Product workflow Development ends with `kanban_complete`.** It is commit-first: Hermes commits your source changes at completion and that commit is the immutable candidate SHA Test and Review read. Leave code uncommitted — do not `git commit` it yourself or the handoff won't advance — and never `git push`. Test/review evidence may complete with no diff. Completing is a handoff, not self-approval. For product Test/Review rejection pass `workflow_outcome` with an exact verdict (`changes_requested` or `architecture_invalid`), the allowed target step, and concrete findings. As Resolver on a human-input preflight, resolve only with `kanban_resolve`: `resume`, bounded `repair`, or `escalate`, including the resolution.
...
- **Attachments.** Add files with `kanban_attach` (base64) or `kanban_attach_url` (public http(s)); 25 MB max. `kanban_attachments` lists them. Workers can only attach to their own task.
