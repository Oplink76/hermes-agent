...
KANBAN_GUIDANCE = (
    ...
    "4. **Block only after exhausting options.** Before a human/capability "
    "block, try safe alternatives: local docs/code, workspace search, fallback "
    "tools, peer comments, and question narrowing. For product boards, call "
    "`kanban_block(reason=\"...\", kind=\"needs_input\", "
    "attempted_resolutions=[...])`. Hermes takes the first resolution pass; "
    "only if Hermes cannot resolve does the card enter `Blocked` and notify "
    "Slack/human. Don't guess or block just because the first path failed.\n"
    "5. **Finish with the review model encoded by the task graph.** Carry the "
    "structured handoff on the lifecycle transition itself: `summary` is "
    "1\u20133 sentences naming concrete artifacts, `metadata` is machine-readable "
    "facts (`{changed_files: [...], tests_run: N, decisions: [...]}`). "
    "Downstream workers read both via `kanban_show`. Never put secrets / "
    "tokens / raw PII in either \u2014 run rows are durable forever. "
    "If `kanban_show()` lists child IDs, inspect them before choosing the "
    "terminal action. When a pre-created review/QA/release child depends on "
    "your task, call `kanban_complete` \u2014 completion is what releases those "
    "children; never sticky-block that parent for `review-required` (a "
    "convention that exists outside the product workflow only) and never "
    "request same-card review too, as either strands or duplicates the lane. "
    ...
    "`kanban_complete`, returns rework with `kanban_request_changes`, or uses "
    "`kanban_block` only for a genuine external escalation. Review is not a "
    "block, so repeated review cycles never trip unblock-loop detection.\n"
    "   **Product workflow Development ends with `kanban_complete`.** It is "
    "commit-first: Hermes commits your source changes at completion and that "
    "commit is the immutable candidate SHA Test and Review read. Leave code "
    "uncommitted \u2014 do not `git commit` it yourself or the handoff won't "
    "advance \u2014 and never `git push`. Test/review evidence may complete with "
    "no diff. Completing is a handoff, not self-approval. For product "
    "Test/Review rejection pass `workflow_outcome` with an exact verdict "
    "(`changes_requested` or `architecture_invalid`), the allowed target "
    "step, and concrete findings. As Resolver on a human-input preflight, "
    "resolve only with `kanban_resolve`: `resume`, bounded `repair`, or "
    "`escalate`, including the resolution.\n"
    ...
    "- **Attachments.** Add files with `kanban_attach` (base64) or "
    "`kanban_attach_url` (public http(s)); 25 MB max. `kanban_attachments` "
    "lists them. Workers can only attach to their own task.\n"
