...
def _record_kanban_budget_exhausted(
    ...
    Fork behaviour: a product-workflow Development card is parked for PO
    review rather than failed outright. ``handle_development_budget_exhaustion``
    returns False for any card that is not a running product/development task,
    in which case the generic terminal failure below applies. Routing lives
    here so every call site inherits it.
    """
    try:
        ...
        try:
            ...
            if handled:
                ...
                logger.info(
                    ...
                    kanban_task, api_call_count, max_iterations,
                )
        finally:
            try:
                _conn.close()
