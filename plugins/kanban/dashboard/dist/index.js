  ...
  function KanbanPage() {
    ...
    const performMoveTask = useCallback(function (taskId, newStatus, count, summary) {
      ...
      const finalPatch = summary
        ...
        : patch;
      if (count > 1) {
        // Optimistic-concurrency guard: capture each task's expected snapshot
        // BEFORE the optimistic UI update, so a missing row aborts without
        // leaving the board visually moved.
        const bulkIds = Array.from(selectedIds);
        const bulkTasks = bulkIds.map(function (id) { return findTaskInBoard(boardData, id); });
        if (bulkTasks.some(function (task) { return !task; })) {
        ...
        sendBulkMutation(
          ...
          Object.assign({ ids: ids }, finalPatch),
        ).then(function (res) {
      ...
      // Single-task path.
      // Optimistic-concurrency guard: capture the expected snapshot BEFORE the
      // optimistic UI update (same reason as the bulk path above).
    ...
    }, [boardData, loadBoard, board, t, selectedIds]);
    ...
    const applyBulk = useCallback(function (patch, confirmMsg) {
      ...
      const run = function () {
        ...
        const bulkIds = Array.from(selectedIds);
        // Optimistic-concurrency guard: resolve every expected snapshot before
        // any optimistic UI mutation so a stale board aborts cleanly.
        const bulkTasks = bulkIds.map(function (id) { return findTaskInBoard(boardData, id); });
        if (bulkTasks.some(function (task) { return !task; })) {
        ...
        const body = Object.assign({ ids: bulkIds }, finalPatch);
        ...
        sendBulkMutation(bulkTasks, withBoard(`${API}/tasks/bulk`, board), body)
          .then(function (res) {
            const failed = (res.results || []).filter(function (r) { return !r.ok; });
            if (failed.length > 0) {
    ...
    }, [boardData, selectedIds, loadBoard, board, t, kanbanDialogs]);
   ...
   const deleteTask = useCallback(function (taskId) {
     return kanbanDialogs.request({
       ...
       // The DELETE endpoint requires a complete expected-task snapshot, so a
       // missing row must abort rather than issue an unguarded delete.
       const currentTask = findTaskInBoard(boardData, taskId);
       if (!currentTask) {
   ...
   }, [boardData, board, loadBoard, t, kanbanDialogs]);
    ...
    const deleteSelected = useCallback(function (count) {
      ...
      kanbanDialogs.request({
        ...
        // Every DELETE carries its own expected snapshot; abort the whole
        // batch if any row is missing rather than deleting unguarded.
    ...
    }, [boardData, selectedIds, board, loadBoard, t, kanbanDialogs]);
