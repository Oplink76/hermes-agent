...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db):
        ...
        # Read statements run on a connection checked out from the bounded
        # read pool, which is not necessarily ``_conn`` or ``_get_read_conn()``.
        # Wrap ``_read_ctx`` so tracing follows whichever connection it yields.
