...
@pytest.fixture
def fitting_hardware(monkeypatch):
    """Workflow tests need a fixed fitting machine, independent of the CI host."""
    ...
    monkeypatch.setattr(
        "hermes_cli.local_runtime.hardware.probe_budget", lambda **_kwargs: budget
    )
...
def test_quickstart_runs_all_three_legs(client, monkeypatch, tmp_path, fitting_hardware):
...
def test_quickstart_skips_satisfied_legs(client, monkeypatch, fitting_hardware):
