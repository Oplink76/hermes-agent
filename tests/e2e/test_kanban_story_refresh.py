...
@pytest.mark.parametrize(
    ("failure_stage", "with_comment"),
    [("spawn", False), ("spawn", True), ("identity", True)],
)
def test_dispatch_retries_resumed_dirty_story_after_post_claim_spawn_failure(
    ...
    def stamp_or_fail(conn, task: kb.Task):
        nonlocal identity_attempts
        identity_attempts += 1
        if with_comment:
            kb.add_comment(conn, task.id, "operator", "Checking worker startup.")
        if failure_stage == "identity" and identity_attempts == 1:
            raise kb.WorkerRuntimeIdentityError("identity temporarily unavailable")
        return _stamp_executor_event(conn, task)

    def flaky_spawn(task: kb.Task, workspace: str) -> int:
        nonlocal attempts
        attempts += 1
        if failure_stage == "spawn" and attempts == 1:
    ...
    assert second == 4242
    assert attempts == (1 if failure_stage == "identity" else 2)
...
def test_dispatch_does_not_revive_invalidated_resume_after_spawn_failure(
    kanban_home: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    board = "story-refresh-stale-retry"
    _repo, story_id, worktree, _branch = _story_fixture(tmp_path, board)
    ...
    with kb.connect(board=board) as conn:
        ...
        def failed_start(task: kb.Task, workspace: str) -> int:
            attempts.append(task.id)
            (worktree / "partial-start.txt").write_text(
                "unreviewed partial start\n", encoding="utf-8"
            )
            raise RuntimeError("spawn failed after touching its workspace")

        assert kb._spawn_one_v2(
            conn, story_id, board=board, spawn_fn=failed_start
        ) is None
        second = kb._spawn_one_v2(
            conn,
            story_id,
            board=board,
            spawn_fn=lambda task, workspace: attempts.append(task.id) or 4242,
        )
        task = kb.get_task(conn, story_id)
        events = kb.list_events(conn, story_id)

    assert second is None
    assert attempts == [story_id]
    ...
    assert (worktree / "partial-start.txt").read_text(
        encoding="utf-8"
    ) == "unreviewed partial start\n"
