...
def test_local_stt_audio_prep_hides_ffmpeg_window(monkeypatch, tmp_path):
    from tools import transcription_audio
    ...
    monkeypatch.setattr(transcription_audio, "_find_ffmpeg_binary", lambda: "ffmpeg")
    monkeypatch.setattr(transcription_audio, "windows_hide_flags", lambda: _CREATE_NO_WINDOW)
    monkeypatch.setattr(subprocess, "run", fake_run)

    transcription_audio._prepare_local_audio(str(tmp_path / "in.m4a"), str(tmp_path))
