...
def test_confirmed_major_review_has_stable_reason_and_evidence(
    ...
    class MajorReviewer:
        def review(self, **kwargs):
            ...
            root = Path(kwargs["resolution_record"]).parent.parent
            initial = write_conflict_review_attempt(
                ...
                review_kind="initial",
                verdict="major",
                findings=("kanban release gate may be absent",),
                reviewed_at="2026-07-13T09:59:00Z",
            )
            ...
            evidence_references.extend(
                (initial.relative_path, confirmation.relative_path)
            )
            return ConflictReviewReceipt(
                candidate_sha=kwargs["candidate_sha"],
                resolver_backend="codex",
    ...
    assert result.details_artifact == evidence_references[-1]


@pytest.mark.parametrize(
    "evidence_artifacts",
    [(), ("../review.json", f"conflict-reviews/review-{'e' * 64}.json")],
)
def test_untrusted_confirmed_major_evidence_is_invalid_review(
    tmp_path: Path,
    ...
    assert result.failed_gate == "conflict_review"
    assert result.details_artifact is None
