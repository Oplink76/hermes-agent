...
from ops.cloudadvisor.hermes_ops.decision_packet import DecisionPacket, write_decision_packet
...
def test_approve_is_available_only_after_ci_review_and_tests_are_green():
    ...
    assert packet(test_results=({"name": "local-gate", "status": "failed"},)).approve_available is False
...
def test_writer_creates_matching_json_markdown_and_hash(tmp_path: Path):
    ...
    expected = hashlib.sha256(
        artifacts.json_path.read_bytes() + artifacts.markdown_path.read_bytes()
    ).hexdigest()
