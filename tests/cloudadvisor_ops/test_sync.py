...
from ops.cloudadvisor.hermes_ops.sync import SyncConfig, SyncState, run
...
def base_responses(*, backlog: int = 3, merge_returncode: int = 0):
    return {
        ...
        ("git", "merge", "--no-edit", "upstream/main"): (merge_returncode, "", "conflict"),
...
def test_clean_merge_updates_exactly_one_existing_pull_request(tmp_path: Path):
    ...
    assert "--force-with-lease=refs/heads/auto-sync/upstream:old-candidate" in push_calls[0].argv
    assert all(
...
def test_candidate_sha_is_fetched_before_force_with_lease(tmp_path: Path):
    ...
    fetch_index = next(index for index, call in enumerate(runner.calls) if call.argv == fetch)
    push_index = next(
        index for index, call in enumerate(runner.calls) if call.argv[:2] == ("git", "push")
    )
